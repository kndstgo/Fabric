...
df = spark.sql("SELECT max(time) as maximo FROM Silver.minciencia_incremental_pl")
display(df)
...
df.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("Bronze.marca_de_agua")
